...
class LTDBSegLoader(Dataset):
    def __init__(self, data_path, win_size, step, train_split, mode="train"):
        ...
        labels = pd.read_csv(data_path + '/LTDB_label.csv').values[:, 1:]
        labels = labels[:-train_valid_split, :]
        self.test_labels = labels[(int)(len(data) * train_split):]
        train_labels = labels[:(int)(len(data) * train_split)]
...
def get_loader_segment(data_path, batch_size, win_size=100, step=1, train_split=0.6, mode='train', num_workers=15, data_name='SMD'):
    if (data_name == 'SMD'):
        ...
        dataset = LTDBSegLoader(data_path, win_size, step, train_split, mode)
